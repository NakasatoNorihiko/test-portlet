...
import com.aimluck.eip.test.util.BookmarkUtils;
 ...
 *
 */
public class BookmarkSelectData extends
    ALAbstractMultiFilterSelectData<EipTTest, EipTTest> implements ALData {

  /** logger */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(BookmarkSelectData.class.getName());
...
//    categoryList = BookmarkUtils.getCategoryList(rundata);
//  }
  ...
  public ResultList<EipTTest> selectList(RunData rundata, Context context) {
    try {
      if (BookmarkUtils.hasResetFlag(rundata, context)) {
        BookmarkUtils.resetFilter(rundata, context, this.getClass().getName());
        target_keyword.setValue("");
      }
      if (BookmarkUtils.hasResetKeywordFlag(rundata, context)) {
        BookmarkUtils.resetKeyword(rundata, context, this.getClass().getName());
      }
      if (BookmarkUtils.hasResetTargetFlag(rundata, context)) {
        BookmarkUtils.resetTarget(rundata, context, this.getClass().getName());
      }
      target_group_name = BookmarkUtils.getTargetGroupName(rundata, context);
      target_user_id = BookmarkUtils.getTargetUserId(rundata, context);
      target_keyword.setValue(BookmarkUtils.getTargetKeyword(rundata, context));
...
//      }
      ...
      BookmarkResultData rd = new BookmarkResultData();
      rd.initField();
...
//        .getValue());
      rd.setTestName(ALCommonUtils.compressString(
        record.getTestName(),
        getStrLength()));
//      if (!BookmarkUtils.isEmptyDate(record.getStartDate())) {
//        rd.setStartDate(ALDateUtil
//          .format(record.getStartDate(), "yyyy年M月d日(E)"));
//      }
//      if (!BookmarkUtils.isEmptyDate(record.getEndDate())) {
//        rd.setEndDate(ALDateUtil.format(record.getEndDate(), "yyyy年M月d日(E)"));
//      }
//      rd.setState(record.getState().intValue());
//      rd.setStateImage(BookmarkUtils.getStateImage(record.getState().intValue()));
//      rd.setStateString(BookmarkUtils.getStateString(record.getState().intValue()));
//      rd.setPriority(record.getPriority().intValue());
//      rd.setPriorityImage(BookmarkUtils.getPriorityImage(record
//        .getPriority()
//        .intValue()));
//      rd.setPriorityString(BookmarkUtils.getPriorityString(record
//        .getPriority()
//        .intValue()));
...
//      rd.setPublicFlag("T".equals(record.getPublicFlag()));
      // 期限状態を設定する．
//      rd.setLimitState(BookmarkUtils.getLimitState(record.getEndDate()));
  ...
  public EipTTest selectDetail(RunData rundata, Context context)
    ...
    try {
      EipTTest test = BookmarkUtils.getEipTTest(rundata, context, true);
      return test;
